...
/**
 * پیاده‌سازی درگاه بانک ملت بر پایهٔ
 * «راهنمای کاربران: شرح توابع و متدهای درگاه پرداخت اینترنتی بانک ملت» نگارش ۱.۳۸
 *
 * توابع اصلی (بند ۲-۲ مستند):
 *   bpPayRequest → هدایت کاربر (Redirect) → bpVerifyRequest → bpSettleRequest
 * توابع شرایط خاص:
 *   bpInquiryRequest (استعلام) و bpReversalRequest (برگشت وجه)
 */
@Injectable()
export class MellatPaymentService {
  ...
  /** برای یکتا بودن orderId ارسالی به درگاه (جلوگیری از کد ۴۱ = شماره درخواست تکراری) */
  ...
  /**
   * اطلاعات پذیرنده. طبق بند ۲-۱ مستند، شمارهٔ ترمینال و نام کاربری و رمز
   * عبور باید از شرکت به پرداخت دریافت شده باشد؛ نبودِ هر کدام یعنی درخواست
   * با کد ۲۴ (اطلاعات کاربری پذیرنده نامعتبر است) رد می‌شود.
   */
  ...
  /**
   * فراخوانی یکی از توابع SOAP درگاه.
   * در صورت خطا، کلاینت کش‌شده دور ریخته می‌شود تا درخواست بعدی با کلاینت
   * سالم ساخته شود.
   */
  ...
  /**
   * شمارهٔ درخواست پرداخت (orderId) — طبق مستند باید به ازای هر درخواست
   * bpPayRequest یکتا باشد وگرنه کد ۴۱ (شماره درخواست تکراری است) برمی‌گردد.
   *
   * به‌صورت رشتهٔ رقم‌ها نگه داشته می‌شود تا در مراحل بعد (verify/settle)
   * دقت عددی از دست نرود.
   */
   ...
   *
   * orderId و saleReferenceId ملت از نوع long و بزرگ‌تر از
   * Number.MAX_SAFE_INTEGER (۹۰۰۷۱۹۹۲۵۴۷۴۰۹۹۱) هستند؛ اگر با Number() خوانده
   * شوند رقم‌های انتهایی خراب می‌شود و bpVerifyRequest / bpSettleRequest
   * شکست می‌خورند. SOAP این مقادیر را به‌صورت متنی منتقل می‌کند، پس رشتهٔ
   * دقیق ورودی درست‌تری است.
   */
  ...
  // =========================================================
  // bpPayRequest — درخواست تراکنش پرداخت
  // =========================================================
  ...
  async requestPayment(
    ...
    // مبلغ از نوع long و به ریال (ورودی مبلغ سفارش تومان است)
    ...
    const payload = {
      ...
      localDate,
      localTime,
      // دادهٔ توضیحی دلخواه پذیرنده (تا ۱۰۰۰ کاراکتر) — برای ردگیری نزد بانک
      additionalData: `orderId:${orderId}`,
      callBackUrl: callbackUrl,
      /*
       * از نگارش ۱.۲۷ مستند، نوع payerId از Long به String تغییر کرده است.
       * در محیط تستی «حتماً باید صفر وارد شود»؛ در محیط عملیاتی فقط برای
       * پذیرندگانی که کد شناسه پرداخت را فعال کرده‌اند مقدار دیگری می‌گیرد.
       */
    ...
    /*
     * مقدار بازگشتی یک رشتهٔ دو بخشی است: «ResCode,RefId»
  ...
  async verifyPayment(
    ...
    // ثبت اطلاعات callback برای ردگیری (حتی اگر ادامهٔ کار ناموفق شود)
    await this.storeCallbackData(payment, callbackData);

    /*
     * طبق مستند، حتی اگر ResCode برگشتی از درگاه غیر از صفر باشد هم «می‌بایست
     * تابع bpVerifyRequest مجدداً فراخوانی گردد تا پاسخ مناسب (موفق یا تراکنش
     * پیش‌تر verify شده است و یا تراکنش پیش‌تر reverse شده است) دریافت گردد».
     * پس اینجا فقط برای ردگیری لاگ می‌شود و جریان verify قطع نمی‌گردد.
     */
    ...
    /*
     * نکتهٔ امنیتی مهم مستند (بند ۲-۴):
     * «پس از دریافت Call Back، پذیرنده می‌بایست حتماً کنترل نماید RefId و
     *  SaleOrderId دریافتی دقیقاً همان RefId و SaleOrderId دریافتی از متد Pay
     *  مربوط به همان تراکنش باشد. در صورت عدم همخوانی OrderId و RefId، پذیرنده
     *  تراکنش را نامعتبر قلمداد می‌نماید و از فراخوانی متد bpVerifyRequest
     *  خودداری می‌کند.»
     *
     * RefId را خودمان کلید جست‌وجو قرار داده‌ایم؛ SaleOrderId باید مقایسه شود.
     */
    ...
    /*
     * اگر پیش‌تر تأیید شده ولی واریز ناتمام مانده باشد، مستقیم به سراغ
     * bpSettleRequest می‌رویم (فراخوانی دوبارهٔ verify لازم نیست).
     */
    ...
    // ذخیره برای استفاده در تلاش‌های بعدی verify/settle/inquiry/reversal
    payment.saleOrderId = gatewayOrderId;
    payment.saleReferenceId = saleReferenceId;
    await this.paymentRepo.save(payment);

    /*
     * طبق مستند، اجباری برای یکتا بودن orderId در توابع verify/settle/inquiry/
     * reversal وجود ندارد و «برای سهولت کار می‌توان این شماره را با شمارهٔ
     * درخواست خرید (saleOrderId) یکسان وارد کرد».
     */
    const bpPayload = {
      ...this.getCredentials(),
      orderId: gatewayOrderId,
